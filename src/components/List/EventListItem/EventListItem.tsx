...
export default function EventListItem({ event, onEventPress }: EventListItemProps) {
  ...
  const sportTypeIcon = React.cloneElement(sportTypeIconMap[event.sportType], {
    style: { ...sportTypeIconMap[event.sportType].props.style, fontSize: 16, marginLeft: 2 },
  });
  return (
    <TouchableOpacity style={[styles.eventListItem, shadowStyles.shadow]} activeOpacity={0.8} onPress={() => onEventPress(event)}>
      <View style={styles.dateTimeBox}>
        ...
        <View style={styles.timeBox}>
          <Text style={[styles.timeText, { fontSize: 16 }]}>{moment(event.dateTime).format("HH:mm")}</Text>
        </View>
      </View>
      <View style={styles.eventInfoBox}>
        ...
        <View style={styles.userTitleBox}>
          ...
          <Text style={styles.titleText}>{numOfParticipants > 0 ? numOfParticipants.toString() : "0"}</Text>
